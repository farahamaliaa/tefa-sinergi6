        ...
        <tbody>
            @forelse ($lates as $late)
                <tr>
                    ...
                    <td>{{ $late->extracurricularStudent->student->user->name }}</td>
                    <td>{{ $late->extracurricularStudent->extracurricular->name }}</td>
                    <td>{{ $late->created_at ? \Carbon\Carbon::parse($late->created_at)->format('H.i') : '-' }}</td>
                    <td>
