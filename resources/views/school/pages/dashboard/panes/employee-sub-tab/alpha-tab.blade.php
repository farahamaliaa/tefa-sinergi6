<div class="table-responsive rounded-3 mb-4" style="max-height: 400px; overflow-y: auto;">
    <table class="table border text-nowrap customize-table mb-0 align-middle text-center">
        ...
        <tbody>
            @forelse ($alpha_teacher as $alpha)
                <tr>
                    <td>{{ $loop->iteration }}</td>
                    <td>{{ $alpha->model->user->name ?? '-' }}</td>
                    <td>{{ $alpha->model->nip ?? '-' }}</td>
                    <td>{{ $alpha->checkin ? \Carbon\Carbon::parse($alpha->checkin)->format('H.i') : '-' }}</td>
                    <td>
                        <span class="badge bg-light-danger text-danger fw-semibold fs-2">Alfa</span>
                    </td>
                </tr>
            @empty
                <tr>
