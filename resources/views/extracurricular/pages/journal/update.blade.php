...
<div class="card mt-4">
    <div class="card-body">
        <form action="{{ route('extracurricular.journal.update', ['id' => $journal->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @forelse($extracurricularStudents as $index => $esStudent)
                            @php
                                $currentStatus = $attendanceMap->get($esStudent->id)?->status ?? 'hadir';
                            @endphp
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                ...
                                <td>
                                    <div class="d-flex gap-2 flex-wrap justify-content-center">
                                        <input type="radio" name="attendance[{{ $esStudent->id }}]" value="hadir" 
                                            class="attendance-radio" id="hadir_{{ $esStudent->id }}" 
