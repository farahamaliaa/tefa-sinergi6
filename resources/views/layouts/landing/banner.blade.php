<section class="banner_section">
    <div class="container">
        <div class="d-flex justify-content-center">
            <div class="">
                <div class="banner_text mb-5">
                    <div class="ban_inner_text" data-aos="fade-up" data-aos-duration="1500">
                        <span><i class="icofont-rocket-alt-2 text-primary"></i> Website Management Sekolah</span>
                        <h1 class="text-primary"><span>Selamat Datang di</span><br>SINERGI6</h1>
                        <p>Sinergi6 merupakan website untuk mempermudahkan berbagai Sekolah-sekolah. <br> Sinergi6 adalah web management sekolah yang menyediakan Fitur Absensi, Pelanggaran, <br>
                            E-learning, Raport, Ujian, Buku Tamu</p>
                    </div>

                    <form action="{{ route('login') }}" data-aos="fade-up" data-aos-duration="1500">
                        <div>
                            <button class="btn btn_main">Masuk Sekarang <i class="icofont-arrow-right"></i></button>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
</section>
